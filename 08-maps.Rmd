...
trees_shape <- st_read("/Users/bedricks/Downloads/Parks_Tree_Inventory-shp")
```
...
Unlike the `pdx_boundaries` data set, there's not a lot of additional information in this one beyond the geometry (the Portland Maps site has additional data sets including bathymetric and other data). What does this geometry look like?
...
pdx_boundaries %>% ggplot() +
  geom_sf() + 
  geom_sf(data=river_boundaries, fill="blue", size=0.0) + 
  geom_sf(data=grocery_stores, color=alpha("seagreen", 0.45)) +
  stat_density_2d(

...
park_base_map <- get_stadiamap(
  c(left=-122.725, bottom=45.4675, right=-122.7125, top=45.478), # note: in practice, we wouldn't enter these by hand!
  zoom=16,
...
It is probably the fastest way to go from "I have geospatial data" to "a map I can look at", with the caveat that it is meant for quick-and-dirty data exploration, not presentation-quality graphics.
...
`mapview` has _many_ controls, for changing the basemap, to setting colors, etc. Note, also, that it is perfectly happy to deal with projection issues for us! We do not need to manually project or otherwise transform our data- it figures it out on its own.

...
set.seed(8675309) # so we can have reproducible results
...
Keiran Healy has a [lovely and strongly-recommended  essay](https://kieranhealy.org/blog/archives/2021/12/19/comparing-distributions/) discussing different design approaches to this sort of plot!
...
Question to consider: what does the y-axis "normally" represent in a histogram? What does it represent in a density plot?
...
```{r}
# Get simple points at mean plus bars representing standard error of the mean
...
# Note that the errorbars themselves overlapped, so use position_dodge to move them horizontally
...
Another option for showing variability: a "ribbon" visualization, showing the area covered by the error bars. This is not the best example for this geom, and is included here for didactic purposes...
...
We can combine stat_summary with other geoms, such as geom_bar(), in order to get error bars on our bar charts.

I'm not in love with this style of design, but sometimes it's the right choice, or a journal or PI requires it...

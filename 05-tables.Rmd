...
By default, RMarkdown's output of tabular output (dataframes, etc.) is not particularly fancy. 
However, we can tell RMarkdown to use `kable` to format all tabular output, which makes much nicer-looking documents.
To make this happen, update the YAML header in your document and override the `df_print` option, when producing HTML output:
...
Sometimes, when writing a document or preparing a report, you may have tabular information to present that is _not_ already in a dataframe. 
For example, consider that helpful table of vectorized math operators from one of the first labs' slide decks.
...
`gt` is a [relatively new package](https://gt.rstudio.com/reference/cols_align.html) designed for publication-ready figures. It has become my go-to tool for tables thanks to its versatility and ease of use.
...
See the problem? Each row in this long dataframe represents a single sample from a single participant; as such, each participant is represented four (-ish) times, and each `study_id` is getting its own entry as though it was a group identifier or something.

To get a more useful output, we need to tell `CreateTableOne` a bit about what we are looking for:
...
Note that `table1`'s output is _always_ HTML, which may or may not be what you want. 
It has numerous options for customization and formatting, etc.

## `gtsummary`

A more recent contender is `gtsummary`, which integrates well with `gt` (but also plays nicely with other table packages, such as `flextable`).

```{r}
library(gtsummary)
mazes %>% 
  select(ca, viq, dx, activity, content) %>% 
  tbl_summary(by=dx,)
```

Things I like about `gtsummary`:

1. Easy and ergnomic syntax
2. Very customizable
3. Easy to add statistical tests, summary rows/cols, etc.
3. Lots of export options
 
Things I don't like about `gtsummary`:

1. Relatively rigid expectations of how a summary table should look

That's about it for "dislikes", really.
...
Note that the specific _name_ that you must use to refer to the font is not exactly obvious. It is _not_ what we would "normally" think of as being the font name, but rather is the font's "family name". 
Because of some under-the-hood details about the way that fonts work, this is a bit different from what you might see in e.g. a font selection menu in Word. 
You can see a list of all the fonts that R knows about by using the `fonttable()` command, which returns a dataframe of all of your computer's installed fonts:

```{r}
fonttable() %>% glimpse()
```

Because it's a dataframe, we can work with it like any other dataframe:
...
The file will look fine for everybody, it will print fine for everybody and will look good on your poster, it will scale fine to any resolution, and will do all the things a PDF should do, _other_ than allow text extraction.

If this is a dealbreaker for your particular situation, my honest advice is to find a different font to use- pick a TTF font (or figure out how to convert your OTF font to a TTF font, I can point you in that direction), then use `extrafont`, and move on with your life. 
...
Nobody, on their deathbed, has ever wished that they had spent more time debugging mysterious error messages involving _multiple_ obscure pieces of thirty-five-year-old software in order to get a PDF file to behave slightly better. 
...
- Kieran Healy himself has gone down this same rabbit hole, somewhat more recently than I have but [with similar conclusions](https://kieranhealy.org/blog/archives/2025/02/06/kerning-and-kerning-in-a-widening-gyre/). His writeup is very worth reading!